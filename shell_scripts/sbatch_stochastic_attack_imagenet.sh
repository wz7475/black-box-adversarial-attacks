...
# Configuration
logs_dir=${1:-"slurm_logs/stochastic_attack_imagenet"}
CPUS_PER_TASK=${2:-4}
MEM=${3:-"32G"}
...
echo "Starting imagenet stochastic attack sbatch job array submission..."
echo "Parameters:"
...
sbatch <<EOF
#!/bin/bash -l
#SBATCH --job-name=stochastic_attack_imagenet
...
echo "Running stochastic growth attack on ImageNet (microsoft/resnet-18)..."
...
python scripts/stochastic_growth_attack.py \
    ...
    --repeat ${repeat} \
    --alpha 0.1 \
    --output_dir outputs_grouped/stochastic_growth_imagenet_alpha_0.1
